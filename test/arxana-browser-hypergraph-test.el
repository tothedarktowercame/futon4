  ...
  (dolist (spec '((evidence-session . session)
    ...
    (let* ((item (list :type (car spec) :id "demo"))
      ...
      (cl-letf (((symbol-function 'arxana-browser--item-at-point)
                 (lambda () item))
                ((symbol-function 'arxana-browser--ensure-evidence)
                 (lambda () t))
...
(ert-deftest arxana-browser-visit-loads-evidence-support-for-open-sessions ()
  (let* ((item (list :type 'evidence-open-session :id "demo"))
         seen
         ensure-called)
    (cl-letf (((symbol-function 'arxana-browser--item-at-point)
               (lambda () item))
              ((symbol-function 'arxana-browser--ensure-evidence)
               (lambda ()
                 ...
                 t))
              ((symbol-function 'arxana-browser-evidence-open-live-session)
               (lambda (arg) (setq seen arg))))
      (arxana-browser--visit))
    (should ensure-called)
    (should (equal seen item))))

...
(ert-deftest arxana-media-create-ep-staging-creates-directory-metadata-and-browser-context ()
  (let* ((root (make-temp-file "arxana-ep-create-" t))
         (arxana-media-ep-staging-root root)
         (arxana-browser--stack (list (list :view 'media)))
         (render-calls 0))
    (unwind-protect
        (cl-letf (((symbol-function 'read-string)
                   (lambda (&rest _) "EP5 mixes"))
                  ((symbol-function 'arxana-browser--render)
                   (lambda () (setq render-calls (1+ render-calls)))))
          (let* ((dest (arxana-media-create-ep-staging))
                 (meta (arxana-media--read-publication-metadata dest))
                 (context (car arxana-browser--stack)))
            (should (file-directory-p dest))
            (should (equal (file-name-nondirectory (directory-file-name dest))
                           "ep5-mixes"))
            (should (equal (plist-get meta :name) "EP5 mixes"))
            (should (equal (plist-get context :view) 'media-ep-staging-ep))
            (should (equal (plist-get context :label) "EP5 mixes"))
            (should (equal (plist-get context :ep-staging-path) dest))
            (should (= render-calls 1))))
      (ignore-errors (delete-directory root t)))))

(ert-deftest arxana-media-create-ep-staging-rejects-existing-slug ()
  (let* ((root (make-temp-file "arxana-ep-existing-" t))
         (arxana-media-ep-staging-root root)
         (existing (expand-file-name "ep5-mixes" root)))
    (unwind-protect
        (progn
          (make-directory existing t)
          (should-error (arxana-media--create-ep-staging-directory "EP5 mixes")
                        :type 'user-error))
      (ignore-errors (delete-directory root t)))))

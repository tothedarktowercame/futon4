...
| 21 | 2026-08-30 | The standing Lean gate ("no `sorryAx`") checked against two delivered hole-moving reports | R9-D2: standard axioms only, but the file did not carry `#print axioms` — the claim was in the bell, not the artefact. R2-D2: as committed, the proof does not elaborate (recursion depth); when retried with `native_decide` it depends on a generated per-proof axiom no `sorryAx` grep or known-name scan would see. The owner's gate had elaborated one lane's file and not the other's | Two instrument lessons in one: a negative check must name what its instrument can see (row 6 — a `sorryAx` grep cannot see a native axiom), and "checked" must mean *ran*, for every artefact, not the one that was convenient. Gate replaced: the artefact must elaborate at the gate, use named theorems, carry `#print axioms`, and name any non-standard axiom with its reason. Cost: one delivered artefact carrying a failing proof for ~40 min before anyone ran it; found by a reader asking, not by the gate |

;;; spine2-export.el --- Export helpers for spine2.org -*- lexical-binding: t; -*-

;;; Commentary:
;; Provide repeatable commands for exporting `spine2.org` into both the
;; traditional LaTeX/PDF (with inline code) and a concise variant where code
;; blocks are replaced by `code>` markers that link to the tangled files.
...
(defvar arxana-spine2-root-directory nil
  "Cached repo root that contains `spine2.org`.")

(defun arxana-spine2--root ()
  "Locate the repository root (directory that holds spine2.org)."
...
(defun arxana-spine2--spine-file ()
  "Absolute path to spine2.org."
  (expand-file-name "spine2.org" (arxana-spine2--root)))
...
(defun arxana-spine2--default-tangle-path (src-block info)
  "Compute the default tangle target for SRC-BLOCK if it tangles to the Org base file."
...
(defun arxana-spine2--resolve-src-target (src-block info)
  "Return the :tangle target for SRC-BLOCK, or nil if it shouldn't be replaced."
...
(defun arxana-spine2--code-link-src-block (src-block contents info)
  "Export SRC-BLOCK as a `code>` link when it's tangled to a file."
...
(defun arxana-spine2--export (backend output)
  "Export spine2.org through BACKEND to OUTPUT."
...
(defun arxana-spine2-export-standard ()
  "Export spine2.org to spine2.tex using the default LaTeX backend."
  (interactive)
  (arxana-spine2--export 'latex "spine2.tex"))

(defun arxana-spine2-export-code-links ()
  "Export spine2.org to spine2-code-links.tex using placeholder links."
  (interactive)
  (arxana-spine2--export 'arxana-latex-code-links "spine2-code-links.tex"))

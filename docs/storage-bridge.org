...
1. Regenerate / load with `M-x arxana-load`.
...
(`test/arxana-adjacency-test.el`), which stub the HTTP layer and verify
...
are covered by `test/arxana-saving-test.el`, ensuring the Emacs commands
call `/snapshot` and `/snapshot/restore` with the selected scope.  The
Org importer/exporter are covered by `test/arxana-import-test.el` and
`test/arxana-export-test.el`, guaranteeing that `.org` bundles can round-trip
...
4. Run the adjacency unit tests to prove scholium links are mirrored correctly:
   ...
   HOME=$PWD/.home-tmp EMACS_NATIVE_COMPILATION=0 emacs --batch \
     -l dev/bootstrap.el \
     -l test/arxana-adjacency-test.el \
...
The ERT suite in `test/arxana-store-test.el` provides fake

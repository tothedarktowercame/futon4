...
`f2.ui` also exposes WIP lab-session endpoints for fucodex and fuclaude
(run, stream, events, and session state). These are used to stream fulab
events for export tooling.

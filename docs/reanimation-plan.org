...
** M6A — Cleanup & Final Decisions [0/5]
- [ ] Confirm the Xanadu-link scope (transclusion, versioned anchors, provenance).
- [ ] Decide the persistence model (XTDB-only vs local cache and sync rules).
- [ ] Lock the authoring workflow (region-based, scholium-first, or hybrid).
- [ ] Define UI expectations (main-buffer overlays, relation browser, docbook).
- [ ] Record any migration or compatibility callouts for legacy links.

** M7 — Docbook + Manual Export [0/5]
- [ ] Verify docbook browser health (filesystem + XTDB sources) and list any blockers.
- [ ] Finish XTDB docbook ingest pipeline (contract → API → storage).
- [ ] Define canonical export targets (Markdown/HTML/PDF) for non-Emacs readers.
- [ ] Build an export command to render manuals from XTDB (or a snapshot mirror).
- [ ] Document the export workflow + prerequisites in README and XTDB docs.

** M8 — Legacy dataset migration notes [0/2]
- [ ] Define scope for “historical datasets” (which backends and export formats).
- [ ] Produce migration notes or close the milestone if no longer relevant.
...
- [ ] Import/export helpers (`arxana-import`, `arxana-articles-export`).

...
Items deferred until after the basic checklist passes.

1. Code browser docbook matching is too slow when computed per row.
   Persist a code→docbook index/TOC in XTDB, fetch it in one query, and
   refresh it on file changes (same pattern as docbook TOC).

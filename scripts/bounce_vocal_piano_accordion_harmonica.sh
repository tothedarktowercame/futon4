...
#    Keep vocal/piano processing light because they are usually captured together.
ffmpeg -hide_banner -y \
  -i "$t1" -i "$t2" -i "$t3" -i "$t4" \
  -filter_complex "
    [0:a]highpass=f=80,  acompressor=threshold=-20dB:ratio=2:attack=20:release=200, volume=1.15, pan=stereo|c0=c0|c1=c0[v];
    [1:a]highpass=f=90,  lowpass=f=9000, acompressor=threshold=-22dB:ratio=2:attack=15:release=180, volume=0.95, pan=stereo|c0=0.65*c0|c1=0.35*c0[pL];
    [2:a]highpass=f=180, acompressor=threshold=-22dB:ratio=2:attack=10:release=150, volume=0.90, pan=stereo|c0=0.45*c0|c1=0.55*c0[aR];
    [3:a]highpass=f=200, acompressor=threshold=-22dB:ratio=2:attack=10:release=150, volume=0.85, pan=stereo|c0=0.25*c0|c1=0.75*c0[hR];
    [v][pL][aR][hR]amix=inputs=4:normalize=0[m];

...
# Track1: spoken word vocal (recorded with banjo bleed)
...
ffmpeg -hide_banner -y \
  -i "$t1" -i "$t2" -i "$t3" -i "$t4" \
  -filter_complex "
    [0:a]highpass=f=80,  acompressor=threshold=-18dB:ratio=2:attack=20:release=200, volume=1.10, pan=stereo|c0=c0|c1=c0[v];
    [1:a]highpass=f=120, acompressor=threshold=-22dB:ratio=2:attack=15:release=180, volume=0.95, pan=stereo|c0=0.6*c0|c1=0.4*c0[bL];
    ...
    [3:a]highpass=f=180, acompressor=threshold=-22dB:ratio=2:attack=10:release=150, volume=0.90, pan=stereo|c0=0.4*c0|c1=0.6*c0[aR];
    [v][bL][ba][aR]amix=inputs=4:normalize=0[m];

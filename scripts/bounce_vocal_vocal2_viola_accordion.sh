...
ffmpeg -hide_banner -y \
  -i "$t1" -i "$t2" -i "$t3" -i "$t4" \
  -filter_complex "
    [0:a]aformat=channel_layouts=stereo,highpass=f=80,  acompressor=threshold=-20dB:ratio=2:attack=20:release=200, volume=1.15,
         pan=stereo|c0=0.5*c0+0.5*c1|c1=0.5*c0+0.5*c1[v1];

    [1:a]aformat=channel_layouts=stereo,highpass=f=100, acompressor=threshold=-22dB:ratio=2:attack=15:release=180, volume=0.90,
         pan=stereo|c0=0.6*c0+0.4*c1|c1=0.4*c0+0.6*c1[v2];

    [2:a]aformat=channel_layouts=stereo,highpass=f=140, lowpass=f=8000, acompressor=threshold=-20dB:ratio=2.5:attack=20:release=220, volume=0.95,
         pan=stereo|c0=0.8*c0+0.2*c1|c1=0.2*c0+0.8*c1[v3];

    [3:a]aformat=channel_layouts=stereo,highpass=f=180, acompressor=threshold=-22dB:ratio=2:attack=10:release=150, volume=0.85,
         pan=stereo|c0=0.8*c0+0.2*c1|c1=0.2*c0+0.8*c1[v4];

    [v1][v2][v3][v4]amix=inputs=4:normalize=0[m];
